...
def index_post():
    ...
    path = request.files['imagePath'].read()
    import sys
    print(path.decode('ascii'), file=sys.stderr)
    img = cv2.imread(path.decode('ascii'))
    rst = get_predictor(checkpoint_path)(img)

    save_result(img, rst)
    print(rst, file=sys.stderr)

    import pickle
    with open(request.files['resultPath'].read().decode('ascii'), 'wb') as f:
        pickle.dump(rst, f)
        f.close()

    return render_template('result.html', session_id=rst['session_id'])
